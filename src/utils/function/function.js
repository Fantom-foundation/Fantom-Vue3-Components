...
import { isPositiveInt } from '../number/number.js';
...
export function debounce(func, wait = 250, trailing = true, leading = false, maxWait = 0) {
    ...
    return (...args) => {
        ...
        if (maxWait > 0 && Date.now() - prevIntervalCallTime >= maxWait) {
            func(...args);
            prevIntervalCallTime = Date.now();
        }
        ...
        if (wait > 0) {
            timeoutId = setTimeout(() => {
                ...
                if (trailing && Date.now() - prevCallTime >= wait) {
                    func(...args);
                }
            }, wait);
        } else {
