<template>
    <div :id="`${id}-cont`" class="flistbox" :class="classes">
        ...
        >
            <template #loader>
                <div class="flistbox_list_item tea-center"><FDotsLoader /></div>
            </template>
            ...
            <ul
                ...
                :aria-activedescendant="!noAriaActivedescendant ? valueOrNull(activeDescendant) : null"
                :aria-labelledby="valueOrNull(ariaLabeledByIds)"
                :aria-describedby="ariaDescribedByIds"
                :aria-disabled="valueOrNull(disabled)"
                :aria-invalid="valueOrNull(validationState.invalid)"
                :aria-label="valueOrNull(ariaLabel)"
                :aria-multiselectable="valueOrNull(multiselect)"
                :aria-orientation="valueOrNull(orientation)"
                @click="onClick"
                @mousedown.prevent
                @keydown="onKeydown"
                @keyup="onKeyup"
            ...
            >
                <li
                    ...
                    :key="`${item.id}_f`"
                    role="option"
                    :aria-selected="valueOrNull(isItemSelected(item))"
                ...
                <li
                    ...
                    :hidden="valueOrNull(item.__prepend)"
                    class="flistbox_list_item"
                    :class="{
                        'flistbox_list_item-focus': item.id === focusedItem.id,
                        'flistbox_list_item-removable': isItemRemovable(item),
...
import { valueOrNull, setIds } from '../../utils/vue-helpers.js';
...
export default {
    ...
    mixins: [selectMixin, formInputMixin, translationsMixin],

    emits: ['update:value', 'loading', 'component-change', 'item-focus', 'item-remove', 'page-change'],
