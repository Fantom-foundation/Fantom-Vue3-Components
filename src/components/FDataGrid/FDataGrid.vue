    ...
    <div class="fdatagrid" :class="classes">
        ...
        <div class="fdatagrid_table" :style="style">
            <table
                ...
                :aria-labelledby="valueOrNull(labeledBy)"
            ...
            >
                    ...
                    <template #loader>
                        <td :colspan="visibleColumnsNum">
                            <slot name="infs-loader">
                                <div class="tea-center"><FDotsLoader /></div>
                            </slot>
                        </td>
                    </template>
                ...
                <tbody v-else-if="!cLoading">
                    <tr>
                        <td :colspan="visibleColumnsNum">
                            <slot name="no-items">
                                <div class="fdatagrid_noitems">{{ translate('fdatagrid.noItems') }}</div>
                            </slot>
                        </td>
                    </tr>
                </tbody>
...
import { actionHandler, setIds, valueOrNull } from '../../utils/vue-helpers.js';
...
export default {
    ...
    created() {
        ...
        if (this.edModeRow) {
            // const listenItemChange = 'item-change' in this.$attrs;
            const listenItemChange = true;
            ...
            this.$watch(
                ...
                () => {
                    ...
                    this.$emit('update:items', this.getItems());
                },
                { deep: true }
            );
    ...
    methods: {
        ...
        async emitChangeEvent(_type, _data = {}) {
            if (_type === 'reload') {
                this._reload = true;
            }
         ...
         */
        async filtersAction(action = '', value = true) {
            return actionHandler(this.filters, action, value);
        },
         ...
         * @return {Promise<unknown>}
         */
        async inputsAction(action = '', value = true) {
            return actionHandler(this.inputs, action, value);
        },

        /**
         * Set id of FFilters component
         */
        async setFiltersId() {
            await this.filtersAction('getState');
            this._filtersId = this.filters.states.length > 0 ? this.filters.states[0].id : '';
        },
        ...
        async getFilters() {
            ...
            if (this._filtersId) {
                await this.filtersAction('getValues', this._filtersId);
                ret.values = this.filters.values;

                await this.filtersAction('getLastChangedElement', this._filtersId);
            ...
            if (column && column.editable) {
                ...
                if (eCell) {
                    ...
                    if (elem) {
                        ...
                        if (id) {
                            this.inputsAction('activateElement', id);
                        }
                    }
                }
            }
